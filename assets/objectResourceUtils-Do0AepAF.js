...
return proj * (view * vec4(pos, 1.0));
  ...
  `)}let la=class extends ee{constructor(e,r){super(e,"int",F.Pass,(o,i,a)=>o.setUniform1i(e,r(i,a)))}};function ca(t,e){e.hasSymbolColors?(t.include(zs),t.attributes.add(p.SYMBOLCOLOR,"vec4"),t.varyings.add("colorMixMode","mediump float"),t.vertex.code.add(s`int symbolColorMixMode;
vec4 getSymbolColor() {
...
return applyScreenSizePerspectiveScaleFactorVec2(size, screenSizePerspectiveScaleFactor(absCosAngle, distanceToCamera, params));
    ...
    `)):r.code.add(s`vec3 addVerticalOffset(vec3 worldPos, vec3 localOrigin) { return worldPos; }`)}const Hs=br();function Ws(t){t.uniforms.add(new re("verticalOffset",(e,r)=>{const{minWorldLength:o,maxWorldLength:i,screenLength:a}=e.verticalOffset,n=Math.tan(.5*r.camera.fovY)/(.5*r.camera.fullViewport[3]),l=r.camera.pixelRatio||1;return ne(Hs,a*l,n,o,i)}))}function ks(t,e){const r=e.output===T.ObjectAndLayerIdColor,o=e.objectAndLayerIdColorInstanced;r&&(t.varyings.add("objectAndLayerIdColorVarying","vec4"),o?t.attributes.add(p.INSTANCEOBJECTANDLAYERIDCOLOR,"vec4"):t.attributes.add(p.OBJECTANDLAYERIDCOLOR,"vec4")),t.vertex.code.add(s`
...
}`)}}const Ys=Er(1,1,0,1),Xs=Er(1,0,1,1);function Js(t){t.fragment.uniforms.add(new Q("depthTexture",(e,r)=>r.mainDepth)),t.fragment.constants.add("occludedHighlightFlag","vec4",Ys).add("unoccludedHighlightFlag","vec4",Xs),t.fragment.code.add(s`void outputHighlight() {
...
color.a = 1.0;
}`);break;case Z.Mask:o.code.add(s`#define discardOrAdjustAlpha(color) { if (color.a < textureAlphaCutoff) { discard; } else { color.a = 1.0; } }`);break;case Z.MaskBlend:t.fragment.code.add(s`#define discardOrAdjustAlpha(color) { if (color.a < textureAlphaCutoff) { discard; } }`)}}function pa(t,e){const{vertex:r,fragment:o}=t,i=e.hasColorTexture&&e.alphaDiscardMode!==Z.Opaque;switch(e.output){case T.LinearDepth:case T.Shadow:case T.ShadowHighlight:case T.ShadowExcludeHighlight:case T.ObjectAndLayerIdColor:pt(r,e),t.include(mt,e),t.include(qe,e),t.include(ft,e),t.include(qs,e),t.include(Be,e),t.include(ks,e),Qo(t),t.varyings.add("depth","float"),i&&o.uniforms.add(new Q("tex",a=>a.texture)),r.code.add(s`void main(void) {
...
forwardTextureCoordinates();
        ...
        `)}}function tl(t,e){const r=t.fragment;e.hasVertexTangents?(t.attributes.add(p.TANGENT,"vec4"),t.varyings.add("vTangent","vec4"),e.doubleSidedMode===oe.WindingOrder?r.code.add(s`mat3 computeTangentSpace(vec3 normal) {
...
}`))}var vt,ro;(function(t){t[t.RED=0]="RED",t[t.RG=1]="RG",t[t.RGBA4=2]="RGBA4",t[t.RGBA=3]="RGBA",t[t.RGBA_MIPMAP=4]="RGBA_MIPMAP",t[t.R16F=5]="R16F",t[t.RGBA16F=6]="RGBA16F"})(vt||(vt={})),function(t){t[t.DEPTH_STENCIL_TEXTURE=0]="DEPTH_STENCIL_TEXTURE",t[t.DEPTH16_BUFFER=1]="DEPTH16_BUFFER"}(ro||(ro={}));const rl=3e5,or=5e5,fa={required:[]};T.LinearDepth;T.CompositeColor;T.Highlight;const ol={required:[T.Depth,T.Normal]};let al=class extends xo{consumes(){return fa}get usedMemory(){return 0}get isDecoration(){return!1}get running(){return!1}get materialReference(){return null}modify(e){}get numGeometries(){return 0}get hasOccludees(){return!1}queryRenderOccludedState(e){return!1}},il=class extends al{};function va(t,e=!0){t.attributes.add(p.POSITION,"vec2"),e&&t.varyings.add("uv","vec2"),t.vertex.code.add(s`
    void main(void) {
